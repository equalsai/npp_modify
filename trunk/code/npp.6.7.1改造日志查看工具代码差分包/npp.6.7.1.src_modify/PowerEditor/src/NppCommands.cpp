...
void Notepad_plus::command(int id) 
{
	switch (id)
	{
		...
		case IDM_VIEW_QUICK_SEEK:
		{
			if((_pQuickSeek) && (!_pQuickSeek->isClosed()))
			{
				_pQuickSeek->display(false);
				_pQuickSeek->setClosed(true);
				checkMenuItem(IDM_VIEW_QUICK_SEEK, false);
			}
			else
			{
				checkMenuItem(IDM_VIEW_QUICK_SEEK, true);
				launchQuickSeek();
				_pQuickSeek->setClosed(false);
			}
		}
		break;
