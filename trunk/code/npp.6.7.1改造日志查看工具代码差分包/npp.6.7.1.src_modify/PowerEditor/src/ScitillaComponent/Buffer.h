...
private:
	...
	//add by ymj
	void ConvertToTxt(char *pSrc, int srcLen, char *pDest, int &destLen);

	void ConvertRecordNo(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertLayerName(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertLocalTime(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertLogType(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertFileName(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertLineNo(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertFunName(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertThreadNo(char *pSrc, int &srcpos, char *pDest, int &destpos);
	void ConvertContentInfo(char *pSrc, int &srcpos, char *pDest, int &destpos, int contentlen);
...
private:
	...
	int m_curAssertItem;
	int m_curItemLen;
	char m_saveAssertStr[1024*7+ 8];
	int m_ContentLen;
	int m_RecordNo;

	//木有办法，该变量值是在A函数里边被赋值，但是实在B函数里边被使用的。
	int m_convertFlag;

private:
	std::vector<string> m_vecLayerName;
	std::vector<string> m_vecLogType;
	std::vector<string> m_vecFileName;
	std::vector<string> m_vecFunName;
	std::vector<string> m_vecThreadNo;
};
...
public:
// 	void setConvertFlag(int flag) { m_ConvertFlag = flag; }

	//Return Value Description:
	//true : is a Converted File
	//false: not a Converted File
 	bool isConvertFile() { return (m_ConvertFlag == 1 ? true : false); }

	std::vector<string>* getVecLayerName()
	{
		return &m_vecLayerName;
	}

	std::vector<string>* getVecLogType()
	{
		return &m_vecLogType;
	}

	std::vector<string>* getVecFileName()
	{
		return &m_vecFileName;
	}

	std::vector<string>* getVecFunName()
	{
		return &m_vecFunName;
	}

	std::vector<string>*getVecThreadNo()
	{
