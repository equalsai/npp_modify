...
class Finder : public DockingDlgInterface {
	...
	friend class CQuickSeekPanel;
public:

	static Finder* getInstance() { return _pSelf; };
...
//	int processAll(ProcessOperation op, const TCHAR *txt2find, const TCHAR *txt2replace, bool isEntire = false, const TCHAR *fileName = NULL, const FindOption *opt = NULL, int colourStyleID = -1);
	...
	void gotoNextFoundResult(int direction = 0) {if (_pFinder->getInstance()->getIsInit()) _pFinder->getInstance()->gotoNextFoundResult(direction);};
	...
	void beginNewFilesSearch()
	{
		...
		_pFinder->getInstance()->addSearchLine(getText2search().c_str());
	}
	...
	HWND getHFindResults() {
		if (_pFinder->getInstance()->getIsInit())
			return _pFinder->getInstance()->_scintView.getHSelf();
		return NULL;
	}

	void updateFinderScintilla() {
		if (_pFinder->getInstance()->getIsInit() && _pFinder->getInstance()->isCreated() && _pFinder->getInstance()->isVisible())
		{
